...
contract DeployHyperDuel is Script {
    ...
    function run() external returns (HyperChallenge challenge) {
        ...
        challenge = new HyperChallenge(buyInToken, platformFeeBps);

        uint256 length = tokenIdsToEnable.length;
        for (uint256 i; i < length;) {
            challenge.enableTradingToken(tokenIdsToEnable[i]);
            unchecked {
                ++i;
            }
        }
        ...
        console2.log("HyperChallenge deployed at:", address(challenge));
